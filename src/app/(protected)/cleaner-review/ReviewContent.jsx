  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      if (response.success) {
        ...
        let filteredReviews = cleanedReviews;

        if (searchQuery.trim()) {
          const query = searchQuery.toLocaleLowerCase().trim();
          filteredReviews = cleanedReviews.filter(review => {
            const cleanerName = (review?.cleaner_user?.name || "").toLocaleLowerCase()
            const locationName = (review?.location?.name || "").toLocaleLowerCase()

            return cleanerName.includes(query) || locationName.includes(query)
    ...
    fetchReviews();
  }, [filter, date, companyId, searchQuery]);
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="flex items-center gap-2 p-3 bg-white rounded-lg shadow-sm border">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                />
                {searchQuery && (
                  <button
