...
export default function ClientDashboard() {
  ...
  const handleStatClick = (filterKey) => {
    ...
    if (filterKey) {
      router.push(`/cleaner-review?companyId=${companyId}&status=${filterKey}`);
    } else {
          ...
          {isLoading
            ...
            : stats.map((stat, idx) => (
              <motion.div
                ...
                className={`bg-white rounded-xl shadow-sm p-5 flex items-center gap-4 border border-slate-200/60 ${stat.filterKey ? "cursor-pointer" : ""
                  }`}
                whileHover={
                  stat.filterKey
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mt-8 border border-slate-200/60">
          ...
          <ul className="space-y-2">
            {recentActivities.map((activity, idx) => (
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full ${activityColors[activity.type]
                    }`}
                />
