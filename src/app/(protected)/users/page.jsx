    ...
    <>
      ...
      <div className="p-4 sm:p-6 md:p-8 bg-slate-50 min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Table View - shown from sm and up */}
          ...
          {/* Mobile Card View - shown below sm */}
          <div className="block sm:hidden">
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => <CardSkeleton key={i} />)
            ) : filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                <div key={user.id} className="mb-4 p-4 bg-white rounded-xl shadow border border-slate-200">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`ml-auto px-2 py-1 text-xs font-semibold rounded-full ${getRoleColorClass(user)}`}>
                      {getRoleDisplayName(user)}
                    </span>
                  </div>
                  <div className="text-slate-600 text-sm mb-2">{user.email || 'N/A'}</div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => navigateTo(`/users/view/${user.id}?companyId=${companyId}`)}
                      className="p-2 cursor-pointer text-green-600 bg-green-100 rounded-md hover:bg-green-200 transition"
