...
export default function SupervisorAssignmentsPage() {
    ...
    useEffect(() => {
        ...
        if (searchQuery) {
            ...
            filtered = filtered.filter(assignment => {
                const supervisorName = assignment.cleaner_user?.name?.toLowerCase() || '';
                const supervisorEmail = assignment.cleaner_user?.email?.toLowerCase() || '';
                const supervisorPhone = assignment.cleaner_user?.phone?.toLowerCase() || '';
    ...
    const fetchAssignments = async () => {
        ...
        try {
            ...
            const response = await AssignmentsApi.getAssignmentsByLocation(locationId, companyId, 3);
    ...
    const handleView = (cleanerUserId) => {
        router.push(`/assignments/supervisor/${cleanerUserId}?companyId=${companyId}&locationId=${locationId}`);
    };

    const handleAddSupervisor = () => {
        router.push(`/assignments/supervisor/add?companyId=${companyId}&locationId=${locationId}&locationName=${encodeURIComponent(locationName)}`);
    };
    ...
    const confirmDelete = async () => {
        ...
        const supervisorName = deleteModal.assignment.cleaner_user?.name || 'Supervisor';
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="bg-white/10 px-4 py-2 rounded-lg">
                                        <div className="text-white text-sm">
                                            <span className="font-bold text-lg">{filteredAssignments.length}</span>
                        ...
                        {filteredAssignments.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <h3 className="text-lg font-medium text-slate-600 mb-2">
                                    {assignments.length === 0 ? 'No Supervisors Assigned' : 'No Results Found'}
                                </h3>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-slate-50 border-b border-slate-200">
                                            ...
                                            <th className="text-left py-4 px-6 font-semibold text-slate-700 text-sm">Supervisor Name</th>
                                            ...
                                            >
                                                ...
                                                <td className="py-4 px-6">
                                                    <div className="flex justify-center gap-2">
                                                        <button
                                                            onClick={() => handleView(assignment.cleaner_user_id)}
                            ...
                            <div className="bg-slate-50 px-6 py-4 border-t border-slate-200">
                                <div className="flex justify-between items-center text-sm text-slate-600">
                                    <span className="font-medium">
                                        Showing {filteredAssignments.length} of {assignments.length} supervisor{assignments.length !== 1 ? 's' : ''}
                                    </span>
                                    <span>
                            ...
                            <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-xl">
                                ...
                                <div className="flex gap-3 justify-end">
                                    ...
                                    <button
                                        ...
                                        disabled={togglingStatus === statusModal.assignment?.id}
                                        className={`px-4 py-2 text-white rounded-lg transition-colors flex items-center gap-2 font-medium cursor-pointer ${statusModal.assignment?.status?.toLowerCase() === 'assigned'
